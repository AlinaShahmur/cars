...
function appReducer(state = {cars: []}, action) {
    switch(action.type) {
        case "LOAD":
        ...
        case 'DELETE':
            let cars = state.cars
            let index = cars.findIndex(car => car._id === action.payload);
            if (index >= 0) {
        ...
        case 'UPDATE':
            ...
            let i = arr.findIndex(item => item._id === id)
            if (i >= 0) {
                arr[i] = updatedCar
            }
        ...
        case 'DELETE_FROM_STATE': 
            ...
            let j = cars1.findIndex(car => car._id === action.payload);
            if (j >= 0) {
                cars1.splice(j, 1)
            }
        ...
        case 'CANCEL_DELETION': 
            ...
            let m = cars2.findIndex(car => car._id === action.payload);
            if (m >= 0) {
